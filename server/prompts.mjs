...
export function liveInstructions({ chairmanName, facts }) {
  return [
    ...
    '「何の用事ですか」「どなたと行きますか」「何時までですか」のような追加の質問はバックエンドの返答に含まれる。自分で別の質問を足さない。会長の答え（「バーナー点検」「一人で」「3時まで」）も必ず委譲する。',
...
export function decisionInput({ facts, calendar, transcript, utterance, lastSpoken, previousThinking, questionsAsked = 0 }) {
  return [
    ...
    '',
    '# 規則',
    '- 予定を聞いたら、まず足りない情報を聞く。聞く項目は 目的地（どこへ）・用件（何をする）・同行者（誰と）・時間（何時から／何時まで）。言われていないものを 1 回に 1 つだけ聞く（例:「菱和産業ですね。何の用事ですか？」「どなたかと行きますか？」「何時ごろまでですか？」）。',
    '- 追加の質問は 1 件の予定につき最大 2 つ（下の「追加の質問の数」で数える。2 なら質問せず確認へ）。優先順は 用件 → 同行者。時間は、用件と同行者が最初から分かっていて質問枠が余っているときだけ「何時ごろまでですか？」と一度聞く。会長が「特にない」「一人」「わからない」「まだ決まってない」と言ったら、その項目は決まったものとして進み、二度と聞かない。「朝から」「終日」「〜まで」を言われたら時間は聞かない。休みの予定は何も聞かない。',
    '- 「特にない」「一人」「わからない」は title に書かない（同行者なしなら同行者を省く）。客先へ行く予定で用件が無いときの区分は 訪問。',
